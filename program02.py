...
def es2(N, ins):
    turned_on_lights = list(ins).copy()

    sequence = []
    for button_id in range(N)[::-1]:
        if button_id + 1 not in turned_on_lights:
            sequence.append(button_id + 1)
            for light_id in range(N):
                if (button_id + 1) % (light_id + 1) == 0:
                    if light_id + 1 in turned_on_lights:
                        turned_on_lights.remove(light_id + 1)
                    else:
                        turned_on_lights.append(light_id + 1)
